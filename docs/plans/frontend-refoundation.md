...
- **Deployed in FF-085/FF-086, validating natural delivery:** targeted
  `event.update`, complete-parent recovery, a bounded live-message journal
  shared by recovery reads, and bounded delivery/application diagnostics.
  The current public-presentation consumer replaces known events only. Missing
  event membership or a missing parent triggers a complete targeted fixture
  replacement, so React never appends an unordered partial event. Do not make
  clip/discovery changes advance fixture recency. The
